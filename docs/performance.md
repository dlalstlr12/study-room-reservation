...
> **2026-09-15 로컬 재실행**: 이 문서의 로컬 수치가 raw 로그 없이 표로만 옮겨져 있어, 같은
> 스크립트 8개를 로컬에서 다시 돌려 터미널 출력을 그대로 커밋했다:
> [`docs/deploy/loadtest-local/`](./deploy/loadtest-local/). 캐시 효과(4.4배)·락 정합성
> (오버부킹 1건)은 그대로 재현됐지만, `none` 전략의 오버부킹 건수는 이번엔 10건이 아니라
> **1건**이 나왔다 — 처리량이 548 req/s로 그때(761)보다 낮아 레이스 창이 좁았다. 바로 아래
> "오버부킹 건수는 부하 크기에 비례" 관찰과 같은 맥락이다. 자세한 대조는 그 폴더의 README.
