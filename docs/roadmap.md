...
- 특정 시점(`targetAt`)에 이용 중인 예약을 스냅샷으로 조회 → 응모자 목록 생성
  (3단계에서 `Room.status` 제거 → "이용 중" = `RESERVED` 이면서 `startAt <= targetAt < endAt`)
...
- [x] **5단계 — 이벤트 추첨**: 스냅샷 응모 + 시드 기반 재현 가능한 추첨(Redisson 락·상태 가드) + 스케줄러 자동 실행 + `@TransactionalEventListener` → `/topic/lottery` 발표
